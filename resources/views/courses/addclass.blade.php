...
<div class="container">
    @if (isset($id))
    <form method="POST" action="{{ route("updateClassStore", ['id' => $id]) }}" class="container">
        @csrf
        <div class="mt-5 me-5 ms-5">
            ...
            <div class="mb-3">
                ...
                <input type="text" value="{{ $course['name'] }}" name="name" class="form-control" placeholder="Saisir le nom du cours">
            </div>
            <div class="mb-3">
                <label for="" class="form-label">Horaire</label>
                <input type="number" value="{{ $course['schedule'] }}" name="schedule" class="form-control" placeholder="Saisir la masse horaire">
            </div>
            <div class="mb-3">
                <label for="" class="form-label">Coéfficient</label>
                <input type="text" value="{{ $course['coefficient'] }}" name="coefficient" class="form-control" placeholder="Saisir le coefficient">
            </div>
        ...
        <button class="btn btn-success mt-3 " type="submit">Editer</button>
    </form>
    @else
    ...
    <form method="POST" action="{{ route("addClassStore") }}" class="container">
        @csrf
        <div class="mt-5 me-5 ms-5">
            <h2 class="text-center text-muted">Vous pouvez ajouter un cours ici !</h2>
